...
class CountSum
{
	...
	public function goodsCount($buyInfo)
	{
    	...
    	if(isset($buyInfo['goods']['id']) && $buyInfo['goods']['id'])
    	{
    		...
    		foreach($goodsList as $key => $val)
    		{
    			...
    			{
					IError::show(403,"商品：".$val['name']."购买数量超出库存，请重新调整购买数量");
    			}
    	...
    	if(isset($buyInfo['product']['id']) && $buyInfo['product']['id'])
    	{
    		...
    		foreach($productList as $key => $val)
    		{
    			...
    			{
					IError::show(403,"货品：".$val['name']."购买数量超出库存，请重新调整购买数量");
    			}
    ...
    public function direct_count($id,$type,$buy_num = 1,$promo='',$active_id='')
    {
		...
		if($type == 'product')
		{
			...
			$productRow = $model->getObj('pro.id = '.$id.' and pro.goods_id = go.id and go.is_del = 0','pro.sell_price,pro.weight,pro.id as product_id,pro.spec_array,pro.goods_id,pro.store_nums,pro.products_no as goods_no,go.name,go.point,go.exp,go.img');
		...
		else
		{
			...
			$goodsRow   = $model->getObj('id = '.$id.' and is_del = 0','go.name,go.id as goods_id,go.img,go.sell_price,go.point,go.weight,go.store_nums,go.exp,go.goods_no,0 as product_id');
